...
import com.ecommerce.contracts.event.stock.StockCommitFailedEventPayload;
...
public class OutboxServiceImpl implements OutboxService {
    ...
    @Override
    @Transactional(propagation = Propagation.MANDATORY)
    public void publishStockCommitFailedEvent(String aggregateId, Long orderId, String transactionId, String reason) {
        try {
            StockCommitFailedEventPayload payload = new StockCommitFailedEventPayload(orderId, transactionId, reason);

            Outbox outboxEvent = Outbox.builder()
                    .aggregateType(EventConstants.AGGREGATE_STOCK)
                    .aggregateId(aggregateId)
                    .messageType(EventConstants.EVENT_STOCK_COMMIT_FAILED)
                    .messagePayload(objectMapper.writeValueAsString(payload))
                    .build();

            outboxRepository.save(outboxEvent);
            log.warn("STOCK_COMMIT_FAILED_EVENT outbox'a yazıldı. OrderID: {}", orderId);
