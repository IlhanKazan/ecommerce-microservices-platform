...
import type { User, UpdateProfileRequest, Address, CreateAddressRequest } from "../types/user";
...
export const userService = {
    ...
    getMe: async (): Promise<User> => {
        const response = await userApi.get<User>(API_ENDPOINTS.USER.ME);
        return response.data;
    },

    updateProfile: async (data: UpdateProfileRequest): Promise<User> => {
        const response = await userApi.put<User>(API_ENDPOINTS.USER.UPDATE, data);
        return response.data;
    },

    getUserById: async (id: number): Promise<User> => {
        const response = await userApi.get<User>(API_ENDPOINTS.USER.BY_ID(id));
        return response.data;
    },

    getAddresses: async (): Promise<Address[]> => {
        const response = await userApi.get<any[]>(API_ENDPOINTS.USER.ADDRESSES);
        return response.data.map(mapAddress);
    },

    addAddress: async (data: CreateAddressRequest): Promise<Address> => {
        const response = await userApi.post<any>(API_ENDPOINTS.USER.ADDRESSES, data);
        return mapAddress(response.data);
    },

    updateAddress: async (id: number, data: CreateAddressRequest): Promise<Address> => {
        const response = await userApi.put<any>(API_ENDPOINTS.USER.ADDRESS_BY_ID(id), data);
        return mapAddress(response.data);
    },

    deleteAddress: async (id: number): Promise<void> => {
        await userApi.delete(API_ENDPOINTS.USER.ADDRESS_BY_ID(id));
    },

    setDefaultAddress: async (id: number): Promise<void> => {
        await userApi.put(API_ENDPOINTS.USER.DEFAULT_ADDRESS(id));
    },
    ...
    uploadAvatar: async (file: File): Promise<string> => {
        ...
        const response = await userApi.post<any>(API_ENDPOINTS.USER.UPLOAD_PROFILE_IMAGE, formData, {
            headers: {
